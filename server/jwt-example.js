...
const registerUser = (first, last, email, password) =>
  new Promise((res, rej) => {
    ...
    return Users.find({ email })
      .then(user => {
        ...
        const newUser = { first, last, email };
...
const createToken = user =>
  jwt.sign(_.omit(user.toObject(), "password"), config.secret, {
    expiresIn: "10s" // lower value for testing
  });

const createRefreshToken = user => {
  // It doesn't always need to be in the /login endpoint route
  const refreshToken = jwt.sign({ type: "refresh" }, config.secret, {
    expiresIn: "20s" // 1 hour
  });
  return Users.findOneAndUpdate({ email: user.email }, { refreshToken })
    .then(() => refreshToken)
    .catch(err => {
      throw err;
    });
...
const logUserActivity = (user, activity) =>
  LoginActivity.create({ userID: user.id, activityType: activity });
